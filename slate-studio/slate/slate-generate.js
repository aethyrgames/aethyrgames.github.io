...
function slateGenerateDoc(wins, opts) {
  ...
  for (const win of wins) {
    ...
    const b = [
      `// Window: "${slateEscape(win.label || 'My Panel')}" `
        + `${Math.round(win.w)}x${Math.round(win.h)} at ${Math.round(win.x)},${Math.round(win.y)}`,
    ...
    blocks.push({ cls, lines: b, members });
  }

  const out = [
    '// Generated by Slate Studio. One SCompoundWidget per window; host each',
    '// in an SWindow or any panel. Edits inside Construct are round-tripped;',
    '// handler names survive, handler bodies are regenerated.',
